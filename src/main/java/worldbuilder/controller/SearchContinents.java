...
public class SearchContinents extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (req.getParameter("submit").equals("search")) {
            req.setAttribute("continents", continents.getByIdentifierLike(req.getParameter("continentSearchType"), req.getParameter("continentSearchTerm")));
        } else if (req.getParameter("submit").equals("world")) {
            World newWorld = worlds.getById(Integer.parseInt(req.getParameter("id")));
            req.setAttribute("continents", continents.getByIdentifierIntEqual("world", newWorld.getId()));
        } else {
            req.setAttribute("continents", continents.getAll());
        }
